---
title: "`phyr`: An R package for model-based phylogenetic analyses in community ecology"
...
# add site environmental data and species traits
...
pglmm_example = phyr::communityPGLMM(freq ~ 1 + shade + (1 | sp__) + (1 | site) + (1 | sp__@site), 
                     data = dat, cov_ranef = list(sp = phyr::phylotree))
```
...
An ecological community contains multiple species occupying the same geographical area, with all species evolved from the same common ancestor. Therefore, for almost every question about ecological community, we need to consider the evolutionary history of species within that community. For some questions, it is statistically necessary to do so [citations]; for other questions, evolutionary relationships among species can provide additional insights about the ecological community [citations]. Consequently, studies in community ecology that involve phylogenies increased exponentially over the past decades [citations]. Most of these studies fall into one or more of the following groups: 1) phylogenetic diversity of the community, 2) comparative studies of species' traits and their relationships with each other as well as environmental conditions, and 3) community assembly patterns and processes. 

Phylogenetic relationships: importance and challenges

Model-based methods for phylogenetic analysis: advantages and challenges
...
`phyr` is written in R and C++. Most of the functions in `phyr` have been included in other R packages. However, the original versions were all written in pure R and thus are relatively slow. Furthermore, methods behind these functional were developed in the same theoretic framework. It makes more sense to combine them together: thus the `phyr` package. Main functions in `phyr` received new major updates and developments and can be categorized into three groups (Table \@ref(tab:mainFunc)): A) phylogenetic diversity (alpha and beta), B) species' traits (e.g. correlations among traits, relationships between traits and environmental variables), and C) community assembly.
...
Two functions (`cor_phylo` and `binaryPGLMM`) were included in this group. Both of them were included in the R package `ape` [@ape2018] originally.

The function `cor_phylo` computes Pearson correlation coefficients for multiple continuous traits that may have phylogenetic signal, allowing users to also specify measurement error. The underlying methods were originally developed by @zheng2009new. We improved speed by writing many of the internal functions in c++, made the input format more flexible, fixed a mistake that meant measurement error was not included, and added two new features: parametric bootstrapping and the ability to use optimizers from the `nlopt` library [@johnson2014nlopt; @powell2009bobyqa; @nelder1965simplex; @rowan1990functional]. 
...
```{r}
tree = phyr::phylotree
phyr::psv(comm_b, tree) # comm_b is a site by species matrix
phyr::psd(comm_b, tree)
x = phyr::pcd(comm_b, tree, verbose = FALSE)
```
...
microbenchmark::microbenchmark(phyr::pcd(comm = comm_a, tree = phylotree, reps = 1000, verbose = F),
                               ...
                               times = 30)
rt = ape::rtree(n = 1000)
microbenchmark::microbenchmark(phyr::vcv2(rt, corr = T),
                               ape::vcv(rt, corr = T))

microbenchmark::microbenchmark(phyr::psv(comm_a, phylotree, compute.var = T),
                               picante::psv(comm_a, phylotree, compute.var = T))

microbenchmark::microbenchmark(
  phyr::communityPGLMM(pa ~ 1 + shade + (1|sp__) + (1|site) + (1|sp__@site), dat, 
                       family = "binomial", cov_ranef = list(sp = phylotree), REML = F, cpp = T, 
                       optimizer = "bobyqa"),
    phyr::communityPGLMM(pa ~ 1 + shade + (1|sp__) + (1|site) + (1|sp__@site), dat, 
                       family = "binomial", cov_ranef = list(sp = phylotree), REML = F, cpp = T, 
                       optimizer = "Nelder-Mead"),
  phyr::communityPGLMM(pa ~ 1 + shade + (1|sp__) + (1|site) + (1|sp__@site), dat, 
                       family = "binomial", cov_ranef = list(sp = phylotree), REML = F, cpp = F, 
                       optimizer = "Nelder-Mead"),
