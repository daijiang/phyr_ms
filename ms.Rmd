...
author: "Daijiang Li, Russell Dinnage, Lucas Nell, Matthew Helmus, Anthony Ives"
...
read.csv("phyr_func.csv", check.names = FALSE) %>% 
  knitr::kable(caption = "(ref:maunFuncCaption)") #%>%
...
No data were used in this study. `phyr` is available at Github (https://github.com/daijiang/phyr) and CRAN (https://cran.r-project.org/package=phyr).
...
$$b_{0\_{sp}}\sim Gaussian(0,\sigma_{0\_{sp}}^{2}\Sigma_{spp})$$
$$b_{0\_{site}}\sim Gaussian(0,\sigma_{0\_{site}}^{2}\Sigma_{site})$$

$$b_{1}\sim Gaussian(0,\sigma_{1}^{2}\Sigma_{spp})$$

$$b_{2}\sim Gaussian(0,\sigma_{2}^{2}I_{site})$$
$$b_{3}\sim Gaussian(0,\sigma_{3}^{2}\Sigma_{site}\otimes\Sigma_{spp})$$

$$e\sim Gaussian(0,\sigma_{e}^2)$$

Here, we set all coefficients ($\beta_0$ to $\beta_3$) to be 1; we also set variances of all random terms to be 1. $\Sigma_{spp}$ is a variance-covariance matrix converted from the phylogeny. $\Sigma_{site}$ is a vairance-covariance matrix among sites, which was converted from the site spatial distance matrix. $I_{site}$ is an identity matrix so that we treat sites as independent replications. $\Sigma_{site}\otimes\Sigma_{spp}$ a kronecker product and forces closely related species will be more likely to be observed in the same site (i.e. underdispersion). In the above model, different species have different overall abundance (intercept) but closely related species ($b_{0\_{sp}}$) and similar communities ($b_{0\_{site}}$) have similar overall abundance. Abundance of different species also change differently along the environmental gradient; however, closely related species again change similarly in response to environmental change. We included a random term for functional trait; therefore the relationship between functional traits and species abundance varied across sites independently to account for unmeasured environmental variables. For demonstration purposes, we only simulated one functional trait and one environmental variable; in real datasets, multiple environmental variables and multiple functional traits can be included in the model. However, more random terms will need more computational time to finish the model fitting.
