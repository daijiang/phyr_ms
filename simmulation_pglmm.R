...
# equal to Vphy. Another way is to use mvnorm package.
...
# without nested term
z.binary <- pglmm(
  presabs ~ 1 + envi + trait + envi:trait +
...
################################
# An example with bayes=T
z.gaussian <- pglmm(
  abund ~ 1 + (1 | species__) + (1 | site__),
  data = dat,
  cov_ranef = list(species = phy, site = Vspace)
)
summary(z.gaussian)
